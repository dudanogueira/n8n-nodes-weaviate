...
export async function execute(
	...
	try {
		...
		// Build query options (same as regular search)
		...
		// Call collection.generate.nearText instead of collection.query.nearText
		const result = await collection.generate.nearText(queryText, generateOptions, queryOptions);
			...
			return [{
				json: {
					...
					metadata: {
						...
						...buildOperationMetadata('generate:nearText', {
							collectionName,
							queryText,
		...
		return result.objects.map((obj: IDataObject, index: number) => ({
			...
			// Add grouped task result (from groupedTask - same for all objects)
